...
CGFloat const kFSTextViewPlaceholderHorizontalMargin = 5.0; ///< placeholder水平方向边距
...
- (void)initialize
{
    ...
    self.wordsNumLabel.font = self.font;
...
- (UILabel *)wordsNumLabel {
    if (!_wordsNumLabel) {
        _wordsNumLabel = [[UILabel alloc] init];
        _wordsNumLabel.textAlignment = NSTextAlignmentRight;
    }
...
- (void)setMaxLength:(NSUInteger)maxLength
{
    ...
    self.text = self.text;
    self.wordsNumLabel.text = [NSString stringWithFormat:@"0/%lu", maxLength];
}
...
- (void)textDidChange:(NSNotification *)notification
{
    ...
    if (_maxLength != NSUIntegerMax && _maxLength != 0 && self.text.length > 0) {
        self.wordsNumLabel.text = [NSString stringWithFormat:@"%lu/%lu", (unsigned long)self.text.length, (unsigned long)self.maxLength];
